...
use crate::{control::{socket::{self, write_uploader_status, MAX_CONNECTED}, Control}, database::{sqlite, Database}, defaults, network::api::{self, Api}, objects::read};
...
impl Uploader {
    ...
    pub fn run(&self) {
        ...
        loop {
            ...
            let mut upload_api: Option<Api> = None;
                        ...
                        for api in apis {
                            if api.kind() == api::API_TYPE_CHRONOKEEP_REMOTE || api.kind() == api::API_TYPE_CHRONOKEEP_REMOTE_SELF {
                                ...
                                match sq.get_not_uploaded_reads() {
                                    Ok(mut reads) => {
                                        // Copy the API into the api object only if we've found reads.
                                        upload_api = Some(api);
                                        // Copy the reads into a new Vec so we don't block the database mutex.
            ...
            if let Some(api) = upload_api {
                ...
                if to_upload.len() > 50 {
                    ...
                    let num_loops = to_upload.len() / 50;
                    ...
                    while loop_counter < num_loops {
                        ...
                        let slice = &to_upload[start_ix..start_ix+50];
                ...
                } else if to_upload.len() > 0 {
                    //println!("Attempting to upload {} reads.", reads.len());
                    match socket::upload_reads(&http_client, &api, &to_upload) {
                        Ok(count) => {
                            // if we uploaded the correct
                            if count == to_upload.len() {
                                let mut modified_reads: Vec<read::Read> = Vec::new();
                                for read in to_upload {
                                    modified_reads.push(read::Read::new(
                                        read.id(),
                                        String::from(read.chip()),
                                        read.seconds(),
                                ...
                                println!("Error uploading reads. Count doesn't match. {} uploaded, expected {}", count, to_upload.len());
                            }
                        },
                        Err(e) => {
                            if error != true {
