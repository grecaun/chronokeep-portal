        ...
        if let Ok(tx) = conn.transaction() {
            let database_tables = [
                ...
                "CREATE TABLE IF NOT EXISTS results_api (
                    ...
                    token VARCHAR(100),
                    uri VARCHAR(150),
                    UNIQUE (uri, auth_token) ON CONFLICT REPLACE
                );",
            ...
            for table in database_tables {
                if let Err(e) = tx.execute(table, ()) {
                    return Err(DBError::ConnectionError(e.to_string()))
                }
            }
...
impl super::Database for SQLite {
        ...
        if let Ok(mut conn) = self.mutex.lock() {
            ...
            match conn.query_row("SELECT * FROM settings WHERE setting=?1;",
                ...
                Ok(it) => {
                    ...
                    return Err(DBError::ConnectionError(String::from("error parsing version value")))
                },
                Err(rusqlite::Error::QueryReturnedNoRows) => return self.make_tables(&mut conn),
                Err(err) => return Err(DBError::ConnectionError(err.to_string()))
            };
        }
    ...
    fn set_setting(&self, name: &str, value: &str) -> Result<setting::Setting, DBError> {
        ...
        if let Ok(conn) = self.mutex.lock() {
            ...
            match res {
                Ok(_) => return Ok(setting::Setting::new(String::from(name), String::from(value))),
        ...
        if let Ok(conn) = self.mutex.lock() {
            ...
            let results = match stmt.query_map([],
                |row| {
                    ...
                    Ok(r) => r,
                    Err(e) => return Err(DBError::RowError(e.to_string()))
                };
                    ...
                    Ok(r) => {
                        match &r.kind[..] {
                            reader::READER_KIND_ZEBRA => {
                                output.push(Box::new(
                                    zebra::Zebra::new(r.id, r.nickname, r.kind, r.ip_address, r.port)
                                ))
                            },
                            reader::READER_KIND_IMPINJ => return Err(DBError::RowError(String::from("not yet implemented"))),
                            reader::READER_KIND_RFID => return Err(DBError::RowError(String::from("not yet implemented"))),
                            _ => return Err(DBError::RowError(String::from("unknown reader kind specified")))
                        }
                    },
                    Err(e) => return Err(DBError::RowError(e.to_string()))
                }
    ...
    fn save_api(&self, name: &str, kind: &str, token: &str, uri: &str) -> Result<usize, DBError> {
        if let Ok(conn) = self.mutex.lock() {
            match conn.execute(
                "INSERT INTO results_api (nickname, kind, token, uri) VALUES (?1,?2,?3,?4);",
                (name, kind, token, uri)
            ) {
                Ok(num) => return Ok(num),
                Err(e) => return Err(DBError::ConnectionError(e.to_string()))
    ...
    fn save_reads(&self, reads: Vec<read::Read>) -> Result<usize, DBError> {
        if let Ok(mut conn) = self.mutex.lock() {
            if let Ok(tx) = conn.transaction() {
                ...
                for r in reads {
                    match tx.execute(
                        "INSERT INTO chip_reads (chip, seconds, milliseconds, antenna, reader, rssi, status) VALUES (?1,?2,?3,?4,?5,?6,?7);",
