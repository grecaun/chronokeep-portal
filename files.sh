...
if ! [[ -e /etc/systemd/system/${SERVICE_NAME}.service ]] || [[ $FORCE_CREATE == true ]]; then
    ...
    echo "------------------------------------------------"
    sudo echo "[Unit]" | sudo tee /etc/systemd/system/${SERVICE_NAME}.service > /dev/null 2>&1
...
if ! [[ -e /etc/systemd/system/${QUIT_SERVICE_NAME}.service ]] || [[ $FORCE_CREATE == true ]]; then
    ...
    echo "------------------------------------------------"
    sudo echo "[Unit]" | sudo tee /etc/systemd/system/${QUIT_SERVICE_NAME}.service > /dev/null 2>&1
